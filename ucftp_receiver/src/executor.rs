    ...
    fn process_result(&mut self, c: CommandExecutionResult, session_id: u64) {
        match c {
            CommandExecutionResult::Process(child) => {
                eprintln!("successfully executed session {}", session_id);
            ...
            CommandExecutionResult::SuccessNoOp => {
                eprintln!("successfully ignored session {}", session_id)
            }
            ...
            CommandExecutionResult::Error(e) => eprintln!(
                "failed to execute command from session {}, error: {}",
                session_id, e
            ),
            CommandExecutionResult::SessionNotFound => {
                eprintln!("session referenced by session {} not found", session_id)
            }
        }
